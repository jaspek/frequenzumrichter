"""Tests für den Leistungsteil (Gleichrichter, Zwischenkreis, Wechselrichter)."""
...
def test_dclink_regen_raises_voltage():
    """Generatorisches Bremsen (i_load < 0) lädt den Kondensator auf."""
    dc = DCLink(
        capacitance=1e-3,
        voltage=560.0,
    ...
    for _ in range(1000):
        dc.update(i_load=-10.0, dt=1e-4)  # Rückspeisung
    assert dc.voltage > 560.0


def test_dclink_motoring_draws_from_source():
    """Beim Motorbetrieb sinkt die Spannung leicht und die Quelle speist nach."""
    dc = DCLink(
        capacitance=1e-3,
        voltage=560.0,
        nominal_voltage=560.0,
...
def test_inverter_duties_in_range_and_voltage_tracks():
    ...
    for d in out.duties:
        assert 0.0 <= d <= 1.0
    # im linearen Bereich folgt die Ist-Spannung dem Sollwert
...
def test_inverter_power_balance_sign():
    """Positive Wirkleistung => positiver Zwischenkreisstrom."""
    inv = Inverter()
    # Strom in Phase mit Spannung -> Leistungsaufnahme
    out = inv.modulate(100.0, 0.0, 540.0, i_alpha=5.0, i_beta=0.0)
    assert out.i_dc > 0.0
    # Strom gegenphasig -> Rückspeisung
...
def test_inverter_saturates_above_linear_range():
    """Oberhalb des Aussteuerkreises wird die Ausgangsspannung begrenzt."""
    inv = Inverter()
    v_dc = 540.0
    out = inv.modulate(1000.0, 0.0, v_dc)  # weit über V_dc/√3
